...
def job_wakeup():
    ...
    url = 'https://github-to-heroku20211031.herokuapp.com/'
    r = requests.get(url)
    print(r)

def send_line(msg, token='THJGFGr2ebX7iZL4o0DZhd10ITJu6lrIY0jGkXorc6B'):
    url = "https://notify-api.line.me/api/notify"  # --> 不支援http, 只能用https
    headers = {"Authorization" : "Bearer "+ token}
    title = '奕忛TERESA空汙排程測試'
...
def job_function2():
    ...
    for item in records:
        if item['County']=='雲林縣' and item['SiteName']=='斗六':
...
def job_function3():
    ...
    resultProxy = my_db.execute("CREATE TABLE IF NOT EXISTS teresa_table (uuid text NOT NULL, time text NOT NULL, aqi text, pm25 text)")
    ...
    for item in records:
        if item['County']=='雲林縣':
            ...
            # insert
            resultProxy=my_db.execute("insert into teresa_table (uuid, time, aqi, pm25) values('%s', '%s', '%s', '%s')" %(uuid, my_time, aqi, pm25))

    # get data from db
    resultProxy=my_db.execute("select * from teresa_table")
...
def start_scheduler():
    ...
    # 每天早上6:30執行
    #scheduler.add_job(job_function2, 'cron', hour='6', minute='00')
    scheduler.add_job(job_function2, 'cron', minute='*/30')
    #scheduler.add_job(job_function2, 'cron', minute='*/120')
